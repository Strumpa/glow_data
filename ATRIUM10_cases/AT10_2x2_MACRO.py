from glow.geometry_layouts.cells import RectCell
from glow.geometry_layouts.geometries import Rectangle
from glow.support.types import GeometryType, PropertyType, SymmetryType
from glow.geometry_layouts.lattices import Lattice
from glow.main import TdtSetup, analyse_and_generate_tdt
from glow.interface.geom_interface import *
from glow.support.types import *


tracking_type = "TISO" # for IC : TISO + MACRO definition
pitch = 1.295
# Build the cell1's geometry layout by adding three circular regions
# Test : assign 1 MACRO per individual physical cell? or safe to assume C2 and C2sym are the same MACRO ?
cell1 = RectCell(name="C1", height_x_width=(pitch, pitch), center=(pitch/2, pitch/2, 0.0))
cell2 = RectCell(name="C2", height_x_width=(pitch, pitch), center=(pitch/2, pitch/2, 0.0))
cell3 = RectCell(name="C2_symm", height_x_width=(pitch, pitch), center=(pitch/2, pitch/2, 0.0))
cell4 = RectCell(name="C4", height_x_width=(pitch, pitch), center=(pitch/2, pitch/2, 0.0))
radii = [0.313602, 0.396678, 0.43227, 0.4435, 0.4520, 0.5140]
#radii = [0.4435, 0.4520, 0.5140]
for radius in radii:
    cell1.add_circle(radius)
    cell2.add_circle(radius)
    cell3.add_circle(radius)
    cell4.add_circle(radius)
# Assign the materials to each zone in the cell1
cell1.set_properties(
      {PropertyType.MATERIAL: ["24UOX_1", "24UOX_1", "24UOX_1", "24UOX_1", "GAP", "CLAD", "MODERATOR"],
       PropertyType.MACRO: ["CELL1", "CELL1", "CELL1", "CELL1", "CELL1", "CELL1", "CELL1"]}
)
cell2.set_properties(
      {PropertyType.MATERIAL: ["32UOX_2", "32UOX_2", "32UOX_2", "32UOX_2", "GAP", "CLAD", "MODERATOR"],
       PropertyType.MACRO: ["CELL2", "CELL2", "CELL2", "CELL2", "CELL2", "CELL2", "CELL2"]}
)
cell3.set_properties(
           {PropertyType.MATERIAL: ["32UOX_3", "32UOX_3", "32UOX_3", "32UOX_3", "GAP", "CLAD", "MODERATOR"],
       PropertyType.MACRO: ["CELL3", "CELL3", "CELL3", "CELL3", "CELL3", "CELL3", "CELL3"]}
)
cell4.set_properties(
      {PropertyType.MATERIAL: ["45UOX_4", "45UOX_4", "45UOX_4", "45UOX_4", "GAP", "CLAD", "MODERATOR"],
       PropertyType.MACRO: ["CELL4", "CELL4", "CELL4", "CELL4", "CELL4", "CELL4", "CELL4"]}
)
# Apply the cell1's sectorization
#cell1.sectorize([1, 1, 1, 1, 1, 1, 8], [0, 0, 0, 0, 0, 0, 22.5], windmill=True)

# --------------------
# LATTICE CONSTRUCTION for TEST of 4 MACROS
# --------------------
# Build the lattice with several rings of the same cartesian cell1
lattice_test = Lattice([cell1], 'ATRIUM-10 2x2', center=(0.0, 0.0, 0.0))
#lattice.add_rings_of_cells(cell1, 3)
lattice_test.add_cell(cell2, ((3/2)*pitch, (1/2)*pitch, 0.0))
lattice_test.add_cell(cell4, ((3/2)*pitch, (3/2)*pitch, 0.0))
lattice_test.add_cell(cell3, ((1/2)*pitch, (3/2)*pitch, 0.0))

lattice_test.show(PropertyType.MATERIAL)

# Assemble all the geometric shapes together
# Update the box cell1's technological geometry with the assembled one
lattice_test.show(PropertyType.MATERIAL)

# Apply the eighth symmetry type to the cartesian lattice
lattice_test.apply_symmetry(SymmetryType.FULL)
# Show the resulting layout with the 'MATERIAL' colorset
lattice_test.show(PropertyType.MATERIAL)
lattice_test.show(PropertyType.MACRO)

if tracking_type == "TISO":
      lattice_test.type_geo = LatticeGeometryType.ISOTROPIC
      analyse_and_generate_tdt(
      [lattice_test], "data/glow_data/tdt_data/AT10_2x2_TISO_MACRO", TdtSetup(
                                                            GeometryType.SECTORIZED, 
                                                            property_types=[PropertyType.MATERIAL, PropertyType.MACRO],
                                                            type_geo=LatticeGeometryType.ISOTROPIC,
#                                                            symmetry_type=BoundaryType.AXIAL_SYMMETRY
                                                            ))