...
def discretize_box_for_MOC(assembly_box_cell, pincell_pitch, assembly_pitch, control_cross_thickness, control_cross_half_span):
    """
    ...
    for rectangle, (nx, ny) in zip(rectangles_to_split, nx_ny_splits):
        splitting_faces.extend(make_grid_faces(rectangle, nx, ny))
...
assembly_box_cell = discretize_box_for_MOC(assembly_box_cell, pin_pitch, assembly_pitch, blade_thickness, blade_half_span)
...
if tracking_type == "TISO":
    output_file_name = "GE-14_assembly_ctrl_MOC_TISO"
    ...
    analyse_and_generate_tdt(
    [lattice], "data/glow_data/tdt_data/GE-14_assembly_ctrl_MOC_TSPC", TdtSetup(GeometryType.SECTORIZED, 
                                                            property_types=[PropertyType.MATERIAL],
