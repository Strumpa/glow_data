# Peach Bottom Unit 2 Type 6 assembly geometry generation using DragonCalculationScheme.
#
# R.Guasch — 24/03/2026
# ---------------------------------------------------------------------------
...
path_to_procs = f"glow_data/BWRProgressionProblems/PB2/{assembly_id}/cle2000_procs"
...
# =====================================================================
PB2_Type6_assembly = CartesianAssemblyModel(
    name=assembly_id,
    tdt_file=f"{path_to_tdt}/{assembly_id}.tdt",
    geometry_description_yaml=path_to_yaml_geometry,
)
PB2_Type6_assembly.set_rod_ID_to_material_mapping(ROD_to_material)
PB2_Type6_assembly.set_uniform_temperatures(
...
PB2_Type6_assembly.analyze_lattice_description(build_pins=True)
PB2_Type6_assembly.set_material_compositions(compositions)
...
for step in scheme.steps:
    ...
    step.apply_radii(PB2_Type6_assembly)
    PB2_Type6_assembly.number_fuel_material_mixtures_by_pin()

    # ---- Build full geometry (fuel cells + box + MACROs) and export TDT ----
    lattice, assembly_box_cell = build_full_assembly_geometry(
        assembly_model=PB2_Type6_assembly,
    ...
    lattice.show(
        ...
        property_type_to_show=PropertyType.MACRO if step.export_macros else PropertyType.MATERIAL,
    )
    ...
    if step.name == "SSH":
        ...
        PB2_Type6_assembly.enforce_material_mixture_indices_from_tdt(tdt_indices)

        # ---- Identify generating / daughter mixes ----
        PB2_Type6_assembly.identify_generating_and_daughter_mixes()

        # ---- Build LIB .c2m ----
        mix_definition_proc_name = f"MIX_{assembly_id}_{step.name}"
        lib = LIB(PB2_Type6_assembly)
        ...
        edi_compo = EDI_COMPO(PB2_Type6_assembly)
