...
tdt_output_path = "test_data" # modify according to where you wish to save output tdt files
        ...
        try:
            ...
            if corner_cb_geom is not None and not is_degenerate(corner_cb_geom):
                if group_2x2_macros:
                    ...
                    macro_name = f"MACRO{macro_col}{macro_row}"
...
for row in lattice_description:
    ...
    for cell in row:
        ...
        if group_2x2_macros:
            ...
            macro_name = f"MACRO{cell_idx}{row_idx}"
...
for col_idx in range(n_cols):
    ...
    if group_2x2_macros:
        ...
        macro_name = f"MACRO{col_idx}{n_rows-1}"
    rect_params = (
        macro_name,
        strip_height,
        pin_pitch,
...
for row_idx in range(n_rows):
    ...
    if group_2x2_macros:
        ...
        macro_name = f"MACRO{n_cols-1}{row_idx}"
    rect_params = (
        macro_name,
        pin_pitch,
        strip_width,
...
if group_2x2_macros:
    ...
    macro_name = f"MACRO{n_cols-1}{n_rows-1}"
...
if group_2x2_macros:
    test_suffix = "2x2_groupings"
else:
    test_suffix = "group_strips_with_cells"

export_layout_to_tdt(
    assembly, f"{tdt_output_path}/PB2_Type6-C_{symmetry_id}_IC_MACRO_TISO_macrodef_test_{test_suffix}", 
