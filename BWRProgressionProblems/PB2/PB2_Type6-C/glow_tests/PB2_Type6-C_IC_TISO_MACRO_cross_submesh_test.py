...
for element_name in cross_elements.keys():
    ...
    macro = cross_elements[element_name]["macro"]
...
assembly.geometry_maps[GeometryType.TECHNOLOGICAL] = wrap_shape(assembly.geom_obj)
## Update the SECTORIZED geometry map with the subdivided regions.
assembly.geometry_maps[GeometryType.SECTORIZED] = \
assembly.get_geometry_map(GeometryType.TECHNOLOGICAL) // wrap_shape(partitioned_face)
...
assembly.show(GeometryType.SECTORIZED, PropertyType.MACRO)

## Export the lower diagonal applying TISO conditions and exporting both MATERIAL and MACRO properties

export_layout_to_tdt(
    assembly, f"{tdt_output_path}/PB2_Type6-C_{symmetry_id}_IC_MACRO_TISO_cross_submesh_test", 
