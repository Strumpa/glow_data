...
## Export the lower diagonal applying TISO conditions and exporting MATERIAL properties
if tip_radius == 0.0:
    if make_cross_solid:
            tdt_file_name = f"PB2_Type6-C_solid_not_rounded_{symmetry_id}_MOC_TSPC_glow_test"
    else:
        tdt_file_name = f"PB2_Type6-C_not_rounded_{symmetry_id}_MOC_TSPC_glow_test"
else:
    if make_cross_solid:
        tdt_file_name = f"PB2_Type6-C_solid_{symmetry_id}_MOC_TSPC_glow_test"
    else:
        tdt_file_name = f"PB2_Type6-C_{symmetry_id}_MOC_TSPC_glow_test"
